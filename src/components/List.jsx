        ...
        <div className="team-list w-full">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
                {teamMembers.map((member, index) => (
                    <div key={index} className="team-member bg-white border border-gray-200 rounded-lg p-4 sm:p-6 hover:shadow-lg transition-shadow duration-300">
                        <h3 className="text-xl sm:text-2xl font-[SansitaBold] text-gray-800 mb-2">{member.name}</h3>
                        <p className="text-base sm:text-lg font-[Sansita] text-gray-600">{member.role}</p>
                    </div>
                ))}
            </div>
        </div>
